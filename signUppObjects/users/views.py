...
class ForgetView(View):
    ...
    def post(self,request):
        ...
        if forget_form.is_valid():
            ...
            try:
                VerifyRecord = TelNumVerifyRecord.objects.filter(telnum=telphone, code=code,send_type=send_type).order_by('-add_time')[0]
                five_munite_ago = datetime.now() - timedelta(hours=0,minutes=5,seconds=0)
                if VerifyRecord:
                    if VerifyRecord.add_time  < five_munite_ago:
...
class GetveryCode(View):
    ...
    def post(self,request):
        ...
        VerifyRecord = TelNumVerifyRecord.objects.filter().order_by('-add_time')[0]
        four_munite_ago = datetime.now() - timedelta(hours=0,minutes=4,seconds=0)
        if VerifyRecord:
            if VerifyRecord.add_time > four_munite_ago:
    ...
    def post(self,request):
        ...
        if register_form.is_valid():
            ...
            try:
                ...
                print('【+】该手机可以注册')
            try:
                VerifyRecord = TelNumVerifyRecord.objects.filter(telnum=telphone,code=code,send_type='register').order_by('-add_time')[0]
                if VerifyRecord:
                    five_minute_ago = datetime.now() - timedelta(hours=0,minutes=5,seconds=0)
                    if VerifyRecord.add_time < five_minute_ago:
...
class UserInfoView(View):
    def get(self,request):
        if not request.user.is_authenticated():
            return HttpResponseRedirect('/users/login?j=1')
...
class UserCouponView(View):
    def get(self,request):
        if not request.user.is_authenticated():
            return HttpResponseRedirect('/users/login?j=1')
        user = request.user
        style = request.GET.get('style','onuse')
        if style == 'onuse':
            should_use = []
            #未使用
            coupons = UserCouponView.objects.filter(user=user,status='onused')
        elif style == 'used':
            #已经使用
            coupons = UserCouponView.objects.filter(user=user, status='used')
        elif style == 'tolate':
            #过期
            coupons = UserCouponView.objects.filter(user=user, status='used')
        return render(request,'personalCoupon.html',{'coupons':coupons,'style':style})


class UserOderView(View):
    def get(self, request):
        if not request.user.is_authenticated():
            return HttpResponseRedirect('/users/login?j=1')
